 ...
 *          value to write
 */
void Adafruit_INA219::WriteRegister(uint8_t reg, uint16_t value) {
  INA219_I2CWrite((uint8_t)ina219_i2caddr, (uint16_t)reg, value);
}
 ...
 *          read value
 */
void Adafruit_INA219::ReadRegister(uint8_t reg, uint16_t* ptrvalue) {
  INA219_I2CRead((uint8_t)ina219_i2caddr, (uint8_t)reg,(uint16_t*) ptrvalue);
}
...
void Adafruit_INA219::powerSave(bool on) {
  uint16_t current;
  ReadRegister(INA219_REG_CONFIG, &current);
  uint8_t next;
  if (on) {
  ...
  WriteRegister(INA219_REG_CONFIG, next);
}
...
int16_t Adafruit_INA219::getPower_raw() {
  ...
  ReadRegister(INA219_REG_POWER, &value);
